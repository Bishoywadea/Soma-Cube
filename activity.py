...
class SomaCube(activity.Activity):
    ...
    def _setup_content(self):
        ...
        instruction_label.set_markup(
            "<span foreground='white' size='large'>Click anywhere to close</span>"
        )
        ...
        self.help_overlay.override_background_color(
            Gtk.StateFlags.NORMAL, Gdk.RGBA(0, 0, 0, 0.8)
        )
    ...
    def _setup_video_player(self, max_width, max_height):
        ...
        if self.player is None:
            raise RuntimeError(
                "Could not create 'playbin'—check installation and plugins"
            )
